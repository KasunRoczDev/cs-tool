...
export class ReleasesService {
  ...
  list() {
    return this.pool
      .query(
        `SELECT r.id, r.version, r.name, r.status, r.created_at, r.updated_at,
  ...
  async create(input: { version: string; name?: string }, userId?: string) {
    const { rows } = await this.pool.query(
      `INSERT INTO releases (version, name, created_by)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [input.version, input.name ?? null, userId ?? null],
    );
  ...
  async update(id: string, patch: { name?: string }) {
    const { rows } = await this.pool.query(
      `UPDATE releases SET name = COALESCE($2, name), updated_at = now()
